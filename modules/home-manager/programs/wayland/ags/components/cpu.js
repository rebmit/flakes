...
const parseCpuFrequencies = (cpuinfo, mode = "max", filterIdle = true) => {
  ...
  if (filterIdle) freqs = freqs.filter((x) => x >= 1000);
...
const parseCpuUsage = (stat) => {
  ...
  const diffUsage =
    Math.round((1000 * (diffTotal - diffIdle)) / diffTotal) / 10;
